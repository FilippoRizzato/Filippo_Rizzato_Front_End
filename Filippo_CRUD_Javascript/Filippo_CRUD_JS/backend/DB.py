...
def update(product_id, data):
    cursor, conn = get_connection()  
    cursor.execute("UPDATE rizzato_filippo_api.products SET nome = %s, marca = %s, prezzo = %s WHERE id = %s", (data["data"][0]["attributes"]['nome'], data["data"][0]["attributes"]["marca"], data["data"][0]["attributes"]['prezzo'], product_id))
    conn.commit()
    cursor.close()
    conn.close()
...
if __name__ == "__main__":
    update(4, '{"data":[{"attributes":{"marca":"Adidas","nome":"T-shirt","prezzo":29.99}}]}')
